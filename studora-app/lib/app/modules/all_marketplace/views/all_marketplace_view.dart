...
class AllMarketplaceView extends GetView<AllMarketplaceController> {
  final ScrollController _scrollController = ScrollController();
  AllMarketplaceView({super.key}) {
  ...
  void _showFilterPanel(BuildContext context) {
    Get.to(
      () => AdvancedFilterView(
        ...
        availableCategories: controller.marketplaceCategories,
        onApplyFilters:
            ({
          ...
          children: [
            ...
            Expanded(
              child: Obx(() {
                ...
                if (controller.marketplaceItems.isEmpty) {
                  return _buildEmptyState(theme);
                }
                return _buildResultsList(context, theme);
              }),
            ),
          ],
        ),
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text(
                "Search marketplace, rentals, etc...",
  ...
  Widget _buildResultsList(BuildContext context, ThemeData theme) {
    return ListView.builder(
      ...
      key: ValueKey('marketplace_list_${controller.activeFilterCount}'),
      itemCount:
          controller.marketplaceItems.length +
          (controller.isLoadingMore.value ? 1 : 0),
      padding: const EdgeInsets.fromLTRB(0, 0, 0, 80.0),
      itemBuilder: (context, index) {
        if (index == controller.marketplaceItems.length) {
          return const Padding(
            padding: EdgeInsets.symmetric(vertical: 32.0),
            child: Center(child: CupertinoActivityIndicator()),
          );
        }
        final item = controller.marketplaceItems[index];
        final bool showDateSeparator = _shouldInsertDateSeparator(
          controller.marketplaceItems,
          index,
        );
        return Column(
            ...
            AnimatedFadeSlide(
              ...
              child: Obx(
                () => DetailedListItemCard(
                  ...
                  currentLoggedInUserId: controller.currentLoggedInUserId ?? '',
                  isUserAd: item.sellerId == controller.currentLoggedInUserId,
                  isFavorite: controller.favoriteItemIds.contains(item.id),
                  onFavoriteTap:
                      item.sellerId == controller.currentLoggedInUserId
                      ? null
                      : () => controller.toggleFavoriteMarketplaceItem(item.id),
                ),
              ),
            ),
          ],
  ...
  Widget _buildMinimalDateSeparator(DateTime date, ThemeData theme) {
    ...
    return Padding(
      padding: const EdgeInsets.fromLTRB(16.0, 20.0, 8.0, 8.0),
      child: Text(
        formattedDate,
