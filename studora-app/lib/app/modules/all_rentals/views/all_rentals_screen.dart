...
class AllRentalsView extends GetView<AllRentalsController> {
  final ScrollController _scrollController = ScrollController();
  AllRentalsView({super.key}) {
  ...
  void _showFilterPanel(BuildContext context) {
    Get.to(
      () => AdvancedFilterView(
        ...
        availableCategories: controller.rentalCategories,
        onApplyFilters:
            ({
  ...
  Widget _buildResultsList(BuildContext context, ThemeData theme) {
    return ListView.builder(
      ...
      key: ValueKey('all_rentals_list_${controller.activeFilterCount}'),
      itemCount:
          controller.rentalItems.length +
          (controller.isLoadingMore.value ? 1 : 0),
      padding: const EdgeInsets.fromLTRB(0, 0, 0, 80.0),
      itemBuilder: (context, index) {
        if (index == controller.rentalItems.length) {
              ...
              child: Obx(
                () => DetailedListItemCard(
                  ...
                  onFavoriteTap:
                      ...
                      ? null
                      : () => controller.toggleFavoriteRental(item.id),
                ),
              ),
            ),
          ],
