...
// ==================== GET PUBLIC LISTINGS ====================
const getPublicListings = async (body, log, error) => {
  const {
    listingType,
    collegeId,
    categoryIds,
  ...
  try {
    ...
    throw { status: 500, message: 'Failed to filter listings.' };
  }
};
  ...
  try {
    ...
    switch (action) {
      case 'getPublicListings':
        log(`[ROUTER] Executing action: getPublicListings`);
        result = await getPublicListings(body, log, error);
